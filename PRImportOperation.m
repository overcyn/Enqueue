...
- (void)main
{
    ...
    if (playWhenDone && playURLs) {
        ...
        for (NSURL *i in URLsToPlay) {
            NSIndexSet *files;
            [[[core db] library] files:&files withValue:[i absoluteString] forAttribute:PRPathFileAttribute _error:nil];
            if ([files count] == 1) {
        ...
        [[[core db] playlists] clearPlaylist:[[core now] currentPlaylist] _error:nil];
        for (NSNumber *i in filesToPlay) {
            [[[core db] playlists] appendFile:[i intValue] toPlaylist:[[core now] currentPlaylist] _error:nil];
        }
...
- (void)addFile:(NSURL *)URL
{
    ...
    if (existingFile != 0) {
        ...
        [[[core db] library] value:&prevLastModified forFile:existingFile attribute:PRLastModifiedFileAttribute _error:nil];
...
//        NSNumber *prevSize;
//        [[[core db] library] value:&prevSize forFile:existingFile attribute:PRSizeFileAttribute _error:nil];
//        NSData *checkSum = [PRTagEditor checkSumForFileAtPath:[URL path]];
//        NSData *prevCheckSum;
//        [[[core db] library] value:&prevCheckSum forFile:existingFile attribute:PRCheckSumFileAttribute _error:nil];
//        if ([checkSum isEqualToData:prevCheckSum] && [size isEqualToNumber:prevSize]) {
//            goto cleanup;
//        }
        PRTagEditor *tagEditor = [[[PRTagEditor alloc] initWithFile:existingFile db:[core db]] autorelease];
        [tagEditor updateTagsAndPostNotification:FALSE];
    } else {
        // valid file?
        ...
        [[[core db] library] files:&prevFiles withValue:size forAttribute:PRSizeFileAttribute _error:nil];
        if ([prevFiles count] > 0) {
            ...
            while (i != NSNotFound) {
                ...
                [[[core db] library] value:&prevURLString forFile:i attribute:PRPathFileAttribute _error:nil];
                NSData *prevCheckSum;
                [[[core db] library] value:&prevCheckSum forFile:i attribute:PRCheckSumFileAttribute _error:nil];
                ...
                if (!prevFileExists && [prevCheckSum isEqualToData:checkSum]) {
                    [[[core db] library] setValue:[URL absoluteString] forFile:i attribute:PRPathFileAttribute _error:nil];
                    goto cleanup;
                }
        ...
        PRFile file;
        NSDictionary *attributes = [NSDictionary dictionaryWithObjectsAndKeys:[[NSDate date] description], [NSNumber numberWithInt:PRDateAddedFileAttribute], nil];
