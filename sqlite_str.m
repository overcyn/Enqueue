...
{
    const UInt16 *str1 = sqlite3_value_text16(values[0]); // Entire string must exist in str2
    const UInt16 *str2 = sqlite3_value_text16(values[1]); // String we ar
    ...
    while (1) {
        ...
        while (c1 == 0 && *str1 != 0) {
            c1 = *(str1++);
            temp = lowerCaseTable[c1>>8];
            /* is there a subtable for this upper byte? */
            if (temp != 0) { /* yes, so fold the char */
...
{
    const UniChar *str1 = strA;
    const UniChar *str2 = strB;
    ItemCount length1 = lenA/2;
    ItemCount length2 = lenB/2;
    register UInt16 c1,c2;
    register UInt16 temp;
...
BOOL no_case_begins(void *udp, int lenA, const void *strA, int lenB, const void *strB)
{
    ...
    if (rangeA.location != 0 && rangeB.location == 0 && rangeB.length < 4) {
        CFStringDelete(stringA, CFRangeMake(0, CFStringGetLength(stringA)));
        CFStringDelete(stringB, CFRangeMake(0, CFStringGetLength(stringB)));
...
{
    static CFCharacterSetRef whiteSpace = nil;
    if (!whiteSpace) {
        whiteSpace = CFRetain(CFCharacterSetGetPredefined(kCFCharacterSetWhitespace));
    }
    int index = 0;
