...
- (void)awakeFromNib {
    ...
    [libraryTableView setVerticalMotionCanBeginDrag:NO];
    ...
    for (NSInteger i = 0; i < 3; i++){
        ...
        NSTextFieldCell *cell = [[PRCenteredTextFieldCell alloc] init];
        [cell setFont:[NSFont systemFontOfSize:11]];
        [cell setTruncatesLastVisibleLine:YES];
        [cell setWraps:NO];
        [cell setLineBreakMode:NSLineBreakByTruncatingTail];
    ...
    horizontalBrowser1TableView = [scrollViews[0] documentView];
    horizontalBrowser2TableView = [scrollViews[1] documentView];
    horizontalBrowser3TableView = [scrollViews[2] documentView];
...
- (void)highlightItem:(PRItem *)item {
    ...
    if (dbRow != -1) {
        ...
        [libraryTableView selectRowIndexes:[NSIndexSet indexSetWithIndex:tableRow] byExtendingSelection:NO];
...
- (void)highlightFiles:(NSArray *)items {
    ...
    if ([dbRows count] > 0) {
        ...
        [libraryTableView selectRowIndexes:tableRows byExtendingSelection:NO];
...
- (void)highlightArtist:(NSString *)artist {
    ...
    [libraryTableView selectRowIndexes:[NSIndexSet indexSetWithIndex:row] byExtendingSelection:NO];
...
- (void)deleteAlertDidEnd:(NSAlert *)alert returnCode:(NSInteger)returnCode contextInfo:(void *)contextInfo {
    ...
    [[NSNotificationCenter defaultCenter] postListItemsDidChange:[now currentList]];
    [libraryTableView selectRowIndexes:[NSIndexSet indexSetWithIndex:0] byExtendingSelection:NO];    
}
...
- (void)loadBrowser {
    ...
    if (browserPosition == PRBrowserPositionVertical) {
        ...
        if (![[db playlists] attrForBrowser:2 list:_currentList]) {
            [horizontalBrowserSubSplitview addSubview:_horizontalBrowser3ScrollView];
        } else if (![[db playlists] attrForBrowser:1 list:_currentList]) {
            [horizontalBrowserSubSplitview addSubview:_horizontalBrowser2ScrollView];
            [horizontalBrowserSubSplitview addSubview:_horizontalBrowser3ScrollView];
        } else {
            [horizontalBrowserSubSplitview addSubview:_horizontalBrowser1ScrollView];
            [horizontalBrowserSubSplitview addSubview:_horizontalBrowser2ScrollView];
            [horizontalBrowserSubSplitview addSubview:_horizontalBrowser3ScrollView];
        }
...
- (BOOL)tableView:(NSTableView *)tableView acceptDrop:(id <NSDraggingInfo>)info row:(NSInteger)row dropOperation:(NSTableViewDropOperation)operation {
    ...
    [libraryTableView selectRowIndexes:indexesToSelect byExtendingSelection:NO];
    return YES;
}
