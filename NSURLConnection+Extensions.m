...
{
    [[NSOperationQueue backgroundQueue] addBlock:^{[NSURLConnection _send:request onCompletion:handler];}];
}
...
+ (void)_send:(NSURLRequest *)request onCompletion:(void (^)(NSURLResponse*, NSData*, NSError*))handler
{
    ...
    [[NSOperationQueue mainQueue] addBlock:^{handler(response,data,error);}];
}
